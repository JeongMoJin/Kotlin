...
fun changeNumber() {
    ...
    number = 20 // 하위 스코프에서 상위 멤버에 접근 할 수 있다
...
changeNumber()
println(number)
//internalNumber = 30 상위 스코프에서 하위 스코프 멤버에 접근 할 수 없다
// 하위 -> 상위로 접근은 가능
...
class Numbers(private var number: Int = 10) {
    fun changeNumber() {
        this.number = 100 // Numbers 라는 스코프안에 존재하기 때문에 number에 접근/변경 할 수 있다
    }
    fun whatIsNumber() {
        return this.number
    }
...
// private -> 내가 포함되어 있는 범위 밖에서도 접근/변경 불가능
